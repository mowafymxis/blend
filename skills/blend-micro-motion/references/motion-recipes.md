...
For simple monoline words, author letter paths directly and reveal them in pen order. Keep the readable action separate; the drawing can be a short optional aside. Avoid stretching a font outline's dash offset and calling it writing: that traces the letter's perimeter. For a different application of local illustration motion, [staged illustrations](staged-illustrations.md) explains how a structure, an accent body, and an interior mark act in separate phases inside a stable surface. Handwriting is not required on every link.
