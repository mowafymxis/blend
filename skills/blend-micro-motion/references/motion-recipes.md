...
Choose the effect with [motion direction](../../blend-scroll/references/motion-direction.md#find-the-small-moments) first. A hover scribble is a good fit for some ink-and-paper identities, not a universal premium treatment. Material, target, and user intent matter more than the effect's name.
...
For simple monoline words, author letter paths directly and reveal them in pen order. Keep the readable action separate; the drawing can be a short optional aside. Avoid stretching a font outline's dash offset and calling it writing: that traces the letter's perimeter. For a different application of path drawing, [Margin](https://github.com/mowafymxis/blend/tree/main/examples/margin) redraws a small bulb filament inside a stable collection card. Its other illustrations use distinct part relationships; handwriting is not required on every link.
