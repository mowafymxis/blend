...
For simple monoline words, author letter paths directly and reveal them in pen order. Keep the readable action separate; the drawing can be a short optional aside. Avoid stretching a font outline's dash offset and calling it writing: that traces the letter's perimeter. For a different application of local illustration motion, [Fold](https://github.com/mowafymxis/blend/tree/main/examples/fold) gives paper layers, a small character, and a star separate gestures inside stable surfaces. Handwriting is not required on every link.
