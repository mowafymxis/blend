(() => {
  ...
  function render() {
    ...
    const gather = smooth(phase(p, 0, 0.35));
    const turn = smooth(phase(p, 0.35, 0.75));
    const settle = smooth(phase(p, 0.75, 1));
  ...
  function schedule() {
    if (!frame && enhanced && active && !document.hidden && !destroyed)
  ...
  function rest() {
    mascot.dataset.state = hovered || focused ? "notice" : "idle";
  }
  function attention() {
    if (mascot.dataset.state !== "hello") rest();
  }
  function greet() {
    clearTimeout(timer);
    // Repeated activation extends the response without queuing another animation.
    mascot.dataset.state = "hello";
    status.textContent = "Hello from Dot.";
  ...
  const on = (target, name, fn, options = {}) =>
    target.addEventListener(name, fn, { ...options, signal });
